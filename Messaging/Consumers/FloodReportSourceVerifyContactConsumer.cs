...
public sealed class FloodReportSourceVerifyContactConsumer(
    ILogger<FloodReportSourceVerifyContactConsumer> logger,
    IConfiguration configuration,
    IServiceScopeFactory scopeFactory
) : IInboxConsumer
{
    public string ContractName => nameof(FloodReportSourceVerifyContact);

    public string TopicName => $"{TopicNames.FloodReportSourceVerifyContact}{configuration["Messaging:TopicSuffix"]?.Trim()}";
    ...
    public async Task HandleMessage(ProcessMessageEventArgs args)
    {
        ...
        try
        {
            var message = JsonSerializer.Deserialize<FloodReportSourceVerifyContact>(args.Message.Body, _jsonOptions);
            if (message is null)
            {
            ...
            ISubscribeRecordRepository subscribeRecordRepository = scope.ServiceProvider.GetRequiredService<ISubscribeRecordRepository>();
            var result = await subscribeRecordRepository.UpdateVerificationStatus(message.SubscribeRecordId, false, args.CancellationToken);
